                    ...
                    <form id="gemstoneForm">
                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12 mt-4">
                                <button type="submit" class="btn-calculate" id="submitBtn">Know your Gemstones</button>
                            </div>
                        </div>
                    </form>
                </div>
...
@section('scripts')
    ...
    <script>
        ...
        // Form Submission
        $('#gemstoneForm').on('submit', async function(e) {
            e.preventDefault();
            ...
            $.ajax({
                url: "{{ route('gemstone.recommendation') }}",
                type: "POST",
                data: $(this).serialize(),
                success: function(res) {
                    if (res.status) {
                        $('#calculator-result').html(res.html).fadeIn();
                        btn.prop('disabled', false).text('Know your Gemstones');
                        $('html, body').animate({
